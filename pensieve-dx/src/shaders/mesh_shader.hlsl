...
void main(
  ...
  const Meshlet meshlet = meshlets[group_id];
  ...
  if (group_thread_id < meshlet.vertex_count) {
    ...
    const float4 position_ws = mul(position_os, g_draw_data.model_mtx);
    const float4 position_cs = mul(position_ws, g_draw_data.view_proj_mtx);
    const float3 normal_ws = normalize(mul(normal_os, (float3x3) g_draw_data.normal_mtx));
    float3 tangent_ws = normalize(mul(tangent_os, (float3x3) g_draw_data.model_mtx));
